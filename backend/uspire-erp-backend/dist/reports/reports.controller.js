...
let ReportsController = class ReportsController {
    ...
    async socePresentation(req, dto) {
        (0, report_query_validator_1.assertNoUnsupportedDimensions)(req.query, ['fiscalYear', 'compare']);
        const fiscalYear = Number(dto.fiscalYear);
        if (!Number.isFinite(fiscalYear) ||
            String(fiscalYear) !== String(dto.fiscalYear)) {
            throw new common_1.BadRequestException('Invalid fiscalYear');
        }
    ...
    async exportSoce(req, dto, res) {
        ...
        if (dto.format === 'csv') {
            ...
            await this.audit
                ...
                .catch(() => undefined);
            res.setHeader('Content-Type', 'text/csv');
            res.setHeader('Content-Disposition', `attachment; filename="Statement_of_Changes_in_Equity_${fiscalYear}.csv"`);
            res.send(body);
            return;
        }
        if (dto.format === 'xlsx') {
            ...
            await this.audit
                ...
                .catch(() => undefined);
            res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
            res.setHeader('Content-Disposition', `attachment; filename="Statement_of_Changes_in_Equity_${fiscalYear}.xlsx"`);
            res.send(body);
            return;
        }
        if (dto.format === 'pdf') {
            ...
            await this.audit
                ...
                .catch(() => undefined);
            res.setHeader('Content-Type', 'application/pdf');
            res.setHeader('Content-Disposition', `attachment; filename="Statement_of_Changes_in_Equity_${fiscalYear}.pdf"`);
            res.send(body);
            return;
        }
        throw new common_1.BadRequestException('Export format not available');
