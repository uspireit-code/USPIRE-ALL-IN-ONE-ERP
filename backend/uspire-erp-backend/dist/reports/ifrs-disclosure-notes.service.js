...
exports.IfrsDisclosureNotesService = void 0;
const common_1 = require("@nestjs/common");
const prisma_service_1 = require("../prisma/prisma.service");
const financial_statements_service_1 = require("./financial-statements.service");
const reports_service_1 = require("./reports.service");
let IfrsDisclosureNotesService = class IfrsDisclosureNotesService {
    prisma;
    fs;
    reports;
    constructor(prisma, fs, reports) {
        this.prisma = prisma;
        this.fs = fs;
    ...
    async getClosedPeriodOrThrow(tenantId, periodId) {
        ...
        if (period.status !== 'CLOSED') {
            throw new common_1.BadRequestException('IFRS Disclosure Notes require a closed accounting period.');
        }
        return period;
    }
    async getCurrencyConfigOrThrow(tenantId) {
        ...
        if (!functionalCurrency || !presentationCurrency) {
            throw this.validationError({
                message: 'Missing currency configuration: defaultCurrency is required in Settings.',
    ...
    async getEntityInfoOrThrow(tenantId) {
        const tenant = await this.prisma.tenant.findUnique({
            where: { id: tenantId },
            select: {
        ...
        const missingFields = [];
        if (!organisationName)
            missingFields.push('organisationName');
        if (!legalName)
    ...
    async generateNote(req, params) {
        const tenant = req.tenant;
        if (!tenant)
            throw new common_1.BadRequestException('Missing tenant context');
        const period = await this.getClosedPeriodOrThrow(tenant.id, params.periodId);
        ...
        if (params.noteCode === 'B')
            return this.noteB({ ...currency });
        if (params.noteCode === 'C')
            return this.noteC(req, { from, to, asOf, openingAsOf });
        if (params.noteCode === 'D')
            return this.noteD(req, { from, to });
        if (params.noteCode === 'E')
            return this.noteE(req, { asOf, openingAsOf, from, to });
        if (params.noteCode === 'F')
            return this.noteF(req, { from, to });
        ...
        throw new common_1.NotFoundException('Unknown IFRS note code');
    }
    noteA(params) {
        const entityTable = {
            ...
            rows: [
                {
                    ...
                    summary: 'The financial statements have been prepared in accordance with International Financial Reporting Standards (IFRS).',
                },
                {
                    policy: 'Measurement basis',
                    summary: 'The financial statements have been prepared on the historical cost basis, except where IFRS requires a different measurement basis.',
                },
            ],
        };
        return {
    ...
    noteB(params) {
        const narrative = 'The accounting policies set out below have been applied consistently to all periods presented in these financial statements.';
        const tables = [
            {
                ...
                rows: [
                    {
                        ...
                        summary: 'Revenue is recognised when control of goods or services is transferred to the customer, in an amount that reflects the consideration expected to be received.',
                    },
                    {
                        policy: 'Property, plant and equipment (PPE)',
                        summary: 'Property, plant and equipment are stated at cost less accumulated depreciation and accumulated impairment losses. Depreciation is charged so as to allocate the cost of assets over their estimated useful lives.',
                    },
                    {
                        policy: 'Depreciation methods',
                        summary: "Depreciation is calculated using a systematic basis that reflects the pattern in which the asset's future economic benefits are expected to be consumed. Residual values and useful lives are reviewed at each reporting date.",
                    },
                    {
                        policy: 'Trade and other receivables (impairment)',
                        summary: 'Trade and other receivables are initially recognised at fair value and subsequently measured at amortised cost, net of expected credit losses.',
                    },
                    {
                        policy: 'Cash and cash equivalents',
                        summary: 'Cash and cash equivalents comprise cash on hand and demand deposits. Bank overdrafts are presented separately as liabilities where applicable.',
                    },
                    {
                        policy: 'Income taxes',
                        summary: 'Current tax is the expected tax payable on taxable profit for the period. Deferred tax is recognised on temporary differences between the carrying amounts of assets and liabilities and their tax bases.',
                    },
                    {
    ...
    async noteC(req, params) {
        ...
        if (categories.length === 0) {
            throw new common_1.BadRequestException('Cannot generate PPE note: no fixed asset categories configured.');
        }
        ...
        const disposalJournals = disposalJournalIds.length
            ...
            : [];
        const disposalDateByJournalId = new Map(disposalJournals.map((j) => [j.id, this.utcDateOnly(j.journalDate)]));
        for (const a of assets) {
            if (a.status === 'DISPOSED') {
                if (!a.disposalJournalId) {
                    throw new common_1.BadRequestException(`Cannot generate PPE note: disposed asset '${a.name}' is missing disposalJournalId.`);
                }
                const dd = disposalDateByJournalId.get(a.disposalJournalId);
                if (!dd) {
                    throw new common_1.BadRequestException(`Cannot generate PPE note: disposal journal for asset '${a.name}' is missing or not POSTED.`);
                }
            }
        }
        ...
        const depTo = (assetId, asOf) => {
            const arr = depByAsset.get(assetId);
            if (!arr?.length)
                return 0;
            let s = 0;
            for (const l of arr) {
                if (l.end.getTime() <= asOf.getTime())
                    s += l.amount;
            }
            return this.round2(s);
        };
        const depInPeriod = (assetId, from, to) => {
            const arr = depByAsset.get(assetId);
            if (!arr?.length)
                return 0;
            let s = 0;
            for (const l of arr) {
                if (l.end.getTime() >= from.getTime() &&
        ...
        const disposalDate = (a) => a.disposalJournalId
        ...
        for (const a of assets) {
            ...
            const capDate = a.capitalizationDate
                ...
                : null;
            if (!capDate) {
                throw new common_1.BadRequestException(`Cannot generate PPE note: asset '${a.name}' is missing capitalizationDate.`);
            }
            const dispDate = disposalDate(a);
            const isInServiceAt = (asOf) => {
                if (capDate.getTime() > asOf.getTime())
                    return false;
                if (!dispDate)
                    return true;
            ...
            if (capDate.getTime() >= fromDate.getTime() &&
            ...
            row.depreciation = this.round2(row.depreciation + depInPeriod(a.id, fromDate, toDate));
        }
        ...
        const openingPpeNet = this.round2(bsOpening.assets.rows
            .filter((r) => r.accountCode === 'PPE_NET')
            .reduce((s, r) => s + r.balance, 0));
        const closingPpeNet = this.round2(bsClosing.assets.rows
            .filter((r) => r.accountCode === 'PPE_NET')
            .reduce((s, r) => s + r.balance, 0));
        const ppePurchasesCash = this.round2(cf.investing.rows
            .filter((r) => r.label.toLowerCase().includes('purchase of property'))
            .reduce((s, r) => s + r.amount, 0));
        const ppeProceedsCash = this.round2(cf.investing.rows
            .filter((r) => r.label.toLowerCase().includes('proceeds from sale of property'))
            .reduce((s, r) => s + r.amount, 0));
        const additionsFromFa = this.round2([...rowsByCatId.values()].reduce((s, r) => s + r.additions, 0));
        const additionsFromCf = this.round2(Math.abs(ppePurchasesCash));
        if (additionsFromFa === 0 && additionsFromCf !== 0) {
            throw new common_1.BadRequestException('Cannot reconcile PPE additions: cash flow shows PPE purchases but no assets were capitalised in the fixed asset register for the period.');
        }
        if (additionsFromFa !== 0 && additionsFromCf === 0) {
            throw new common_1.BadRequestException('Cannot reconcile PPE additions: assets were capitalised in the fixed asset register but no PPE purchases were classified in the investing cash flow. Non-cash or unclassified additions detected.');
        }
        this.assertTieOut({
            noteCode: 'C',
            statement: 'CF',
            label: 'PPE additions (fixed asset register) to cash flow investing purchases of PPE',
            expected: additionsFromFa,
            actual: additionsFromCf,
            tolerance: 0.01,
        });
        const additions = additionsFromFa;
        const disposals = this.round2(-1 * [...rowsByCatId.values()].reduce((s, r) => s + r.disposals, 0));
        const depreciationExpense = this.round2(pl.expenses.rows
            ...
            .reduce((s, r) => s + r.balance, 0));
        const depFromFa = this.round2([...rowsByCatId.values()].reduce((s, r) => s + r.depreciation, 0));
        this.assertTieOut({
            noteCode: 'C',
            statement: 'PL',
            label: 'Depreciation for the period (fixed asset register) to depreciation expense (P&L)',
            expected: depFromFa,
            actual: depreciationExpense,
            tolerance: 0.01,
        });
        const depreciation = this.round2(-Math.abs(depreciationExpense));
        const computedClosing = this.round2(openingPpeNet + additions + disposals + depreciation);
        this.assertTieOut({
            noteCode: 'C',
            statement: 'BS',
            label: 'PPE movement (opening + additions + disposals + depreciation) to closing PPE_NET',
        ...
        const totalRow = {
            className: 'Total',
            opening: this.round2([...rowsByCatId.values()].reduce((s, r) => s + r.opening, 0)),
            additions: additions,
            disposals: disposals,
            depreciation: depreciation,
            closing: this.round2([...rowsByCatId.values()].reduce((s, r) => s + r.closing, 0)),
        };
        this.assertTieOut({
            noteCode: 'C',
            statement: 'BS',
        ...
        return {
            ...
            narrative: 'Property, plant and equipment are stated at cost less accumulated depreciation and accumulated impairment losses. The movements in property, plant and equipment for the period are set out below.',
            footnotes: [
    ...
    async noteD(req, params) {
        ...
        if (categories.length === 0) {
            throw new common_1.BadRequestException('Cannot generate depreciation note: no fixed asset categories configured.');
        }
        const fromDate = this.utcDateOnly(new Date(params.from));
        const toDate = this.utcDateOnly(new Date(params.to));
        const assets = await this.prisma.fixedAsset.findMany({
        ...
        const assetIdToCategoryId = new Map(assets.map((a) => [a.id, a.categoryId]));
        const depLines = assets.length
            ...
            : [];
        const depByCatId = new Map();
        for (const c of categories)
            depByCatId.set(c.id, 0);
        for (const l of depLines) {
            const catId = assetIdToCategoryId.get(l.assetId);
            if (!catId)
                continue;
            depByCatId.set(catId, this.round2((depByCatId.get(catId) ?? 0) + Number(l.amount)));
        }
        const depFromFa = this.round2([...depByCatId.values()].reduce((s, n) => s + n, 0));
        const depFromPl = this.round2(pl.expenses.rows
            .filter((x) => this.classifyDepreciationExpense({
            code: x.accountCode,
            name: x.accountName,
            type: 'EXPENSE',
            ...
            .reduce((s, r) => s + r.balance, 0));
        const depFromCf = this.round2(cf.operating.adjustments
        ...
        this.assertTieOut({
            noteCode: 'D',
            statement: 'PL',
            label: 'Depreciation per fixed asset register to depreciation expense (P&L)',
            expected: depFromFa,
            actual: depFromPl,
            tolerance: 0.01,
        });
        this.assertTieOut({
            noteCode: 'D',
            statement: 'CF',
            label: 'Depreciation per Cash Flow adjustment to depreciation expense (P&L)',
            expected: depFromPl,
            actual: depFromCf,
            tolerance: 0.01,
        });
        ...
        return {
            noteCode: 'D',
            title: 'Depreciation',
            narrative: 'Depreciation is charged to profit or loss so as to allocate the depreciable amount of property, plant and equipment over its estimated useful lives. The depreciation charge for the period is analysed below.',
            footnotes: [
    ...
    async noteE(req, params) {
        ...
        const netPerBs = this.round2(bs.assets.rows
            .filter((r) => r.accountCode === 'TRADE_RECEIVABLES')
            .reduce((s, r) => s + r.balance, 0));
        this.assertTieOut({
            noteCode: 'E',
            statement: 'BS',
            label: 'Trade receivables (gross + allowance) to Balance Sheet TRADE_RECEIVABLES (net)',
            expected: netPerBs,
            actual: netComputed,
            tolerance: 0.01,
        });
        ...
        return {
            ...
            narrative: 'Trade receivables are measured at amortised cost and are presented net of expected credit losses. The ageing analysis below is based on amounts outstanding at the reporting date.',
            footnotes: [
                'The closing balance agrees to the Statement of Financial Position.',
            ],
    ...
    async noteF(req, params) {
        ...
        const cashPerBs = this.round2(bs.assets.rows
            .filter((r) => r.accountCode === 'CASH_AND_EQUIVALENTS')
            .reduce((s, r) => s + r.balance, 0));
        const overdraftPerBs = this.round2(bs.liabilities.rows
        ...
        if (cashAccounts.length === 0) {
            throw new common_1.BadRequestException('Cannot generate cash note: no cash/bank accounts found in the ledger as of period end.');
        }
        let cashTotal = 0;
        ...
        this.assertTieOut({
            noteCode: 'F',
            statement: 'BS',
            label: 'Cash total (positive balances) to Balance Sheet CASH_AND_EQUIVALENTS',
            expected: cashPerBs,
            actual: cashTotal,
            tolerance: 0.01,
        });
        this.assertTieOut({
            noteCode: 'F',
            statement: 'BS',
            label: 'Overdraft total (negative balances) to Balance Sheet BANK_OVERDRAFT',
        ...
        this.assertTieOut({
            noteCode: 'F',
            statement: 'CF',
            label: 'Net cash position (BS cash less overdraft) to Cash Flow closing cash',
        ...
        return {
            ...
            narrative: 'Cash and cash equivalents comprise cash on hand and demand deposits. Bank overdrafts are presented separately as liabilities where applicable.',
            footnotes: [
    ...
    async noteG(req, params) {
        ...
        if (soce.from !== params.from || soce.to !== params.to) {
            throw new common_1.BadRequestException('IFRS Disclosure Notes require a closed accounting period that matches the fiscal year range used for the Statement of Changes in Equity.');
        }
        ...
        const otherShareCapital = this.round2(closingShareCapital - openingShareCapital);
        const otherRetainedEarnings = this.round2(closingRetainedEarnings - openingRetainedEarnings - profitOrLoss);
        const otherOtherReserves = this.round2(closingOtherReserves - openingOtherReserves);
        const otherTotal = this.round2(closingEquityTotal - openingEquityTotal - profitOrLoss);
        this.assertTieOut({
            noteCode: 'G',
            statement: 'BS',
            label: 'Equity movement (opening + profit/(loss) + other movements) to closing equity (Balance Sheet)',
        ...
        return {
            noteCode: 'G',
            title: 'Equity',
            narrative: 'The movements in equity for the period are presented in the statement of changes in equity and are summarised below.',
            footnotes: [
                'The closing balance agrees to the Statement of Financial Position.',
            ],
            tables: [table],
    ...
    async noteH(req, params) {
        ...
        if (!tenant)
            throw new common_1.BadRequestException('Missing tenant context');
        const currentTaxExpense = this.round2(pl.expenses.rows
            .filter((r) => r.reportSection === 'TAX_EXPENSE')
            .reduce((s, r) => s + r.balance, 0));
        const profitBeforeTax = this.round2(cf.operating.profitBeforeTax);
        const deferredTaxAsset = this.round2(bs.assets.rows
            .filter((r) => r.accountCode === 'DEFERRED_TAX_ASSET')
            .reduce((s, r) => s + r.balance, 0));
        const deferredTaxLiability = this.round2(bs.liabilities.rows
        ...
        if (incomeTaxAccounts.length === 0) {
            throw new common_1.BadRequestException('Cannot generate income tax note: no income tax control accounts configured. Set Account.ifrsMappingCode to TAX:INCOME (or TAX:CIT) for the current tax payable/receivable control account(s).');
        }
        ...
        const sumNetTo = async (cutoff) => {
            ...
            const out = new Map();
            for (const g of grouped) {
                out.set(g.accountId, this.round2(Number(g._sum?.debit ?? 0) - Number(g._sum?.credit ?? 0)));
            }
            return out;
        };
        const opening = await sumNetTo(this.addUtcDays(fromDate, -1));
        const closing = await sumNetTo(toDate);
        const sumMovementByRefPrefix = async (prefix) => {
            const grouped = await this.prisma.journalLine.groupBy({
                by: ['accountId'],
                where: {
                    accountId: { in: accountIds },
        ...
        this.assertTieOut({
            noteCode: 'H',
            statement: 'PL',
            label: 'Income tax charged per control account reconciliation to tax expense (P&L TAX_EXPENSE)',
            expected: currentTaxExpense,
            actual: chargedTotal,
            tolerance: 0.01,
        });
        const taxExpensePerCf = this.round2(cf.operating.profitBeforeTax - pl.profitOrLoss);
        this.assertTieOut({
            noteCode: 'H',
            statement: 'CF',
            label: 'Current tax expense (P&L TAX_EXPENSE) to cash flow profit-before-tax bridge',
            expected: currentTaxExpense,
            actual: taxExpensePerCf,
            tolerance: 0.01,
        });
        const currentTaxReconciliationTable = {
        ...
        return {
            noteCode: 'H',
            title: 'Income tax',
            narrative: 'Income tax expense comprises current tax and deferred tax. Current tax represents the expected tax payable on taxable profit for the period. Deferred tax is recognised on temporary differences between the carrying amounts of assets and liabilities and their tax bases.',
            footnotes: [
...
exports.IfrsDisclosureNotesService = IfrsDisclosureNotesService;
exports.IfrsDisclosureNotesService = IfrsDisclosureNotesService = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [prisma_service_1.PrismaService,
        financial_statements_service_1.FinancialStatementsService,
        reports_service_1.ReportsService])
], IfrsDisclosureNotesService);
//# sourceMappingURL=ifrs-disclosure-notes.service.js.map