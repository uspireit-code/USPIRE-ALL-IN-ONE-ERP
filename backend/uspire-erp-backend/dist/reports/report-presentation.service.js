...
exports.ReportPresentationService = void 0;
const common_1 = require("@nestjs/common");
const financial_statements_service_1 = require("./financial-statements.service");
function money(n) {
    return Number(n).toFixed(2);
}
let ReportPresentationService = class ReportPresentationService {
    engine;
    constructor(engine) {
        this.engine = engine;
    }
    ...
    async presentPL(req, params) {
        ...
        let compare = null;
        let comparePeriod;
        let compareOmittedReason;
        if (params.compare) {
            const cFrom = this.addMonths(params.from, params.compare === 'prior_month' ? -1 : -12);
            const cTo = this.addMonths(params.to, params.compare === 'prior_month' ? -1 : -12);
            try {
        ...
        const compareBalanceFor = (params) => {
            if (!compare)
                return undefined;
            const rows = params.prefix === 'INC' ? compare.income.rows : compare.expenses.rows;
            return (rows.find((x) => x.accountCode === params.accountCode &&
                x.accountName === params.accountName)?.balance ?? 0);
        };
        const parseCode = (code) => {
            const n = Number(code);
            return Number.isFinite(n) ? n : null;
        };
        const categorize = (row, source) => {
        ...
        const asPresentedRows = (keyPrefix, rows, normal) => rows.map((r) => {
        ...
        const compareTotals = compare
            ? (() => {
                ...
                const cCostOfSales = cExpenses.filter((r) => r.category === 'COST_OF_SALES');
                const cOpEx = cExpenses.filter((r) => r.category === 'OPERATING_EXPENSES');
                const cOtherInc = cIncome.filter((r) => r.category === 'OTHER_INCOME');
                const cOtherExp = cExpenses.filter((r) => r.category === 'OTHER_EXPENSES');
    ...
    async presentBS(req, params) {
        ...
        let compareOmittedReason;
        if (params.compare) {
            const cAsOf = this.addMonths(params.asOf, params.compare === 'prior_month' ? -1 : -12);
            try {
        ...
        const section = (key, label, rows, subtotalLabel, compareRows, compareSubtotal) => ({
            key,
            label,
            rows: rows.map((r) => ({
                ...
                amount: this.present(r.balance),
                compareAmount: compare
                    ? this.present(compareRows?.find((x) => x.accountCode === r.accountCode)
                        ?.balance ?? 0)
                    : undefined,
            })),
            subtotal: {
        ...
        const curAssetsNonCurrent = current.assets.rows.filter((r) => r.reportSection === 'ASSETS_NON_CURRENT');
        const curAssetsCurrent = current.assets.rows.filter((r) => r.reportSection === 'ASSETS_CURRENT');
        const curLiabNonCurrent = current.liabilities.rows.filter((r) => r.reportSection === 'LIABILITIES_NON_CURRENT');
        const curLiabCurrent = current.liabilities.rows.filter((r) => r.reportSection === 'LIABILITIES_CURRENT');
        const cmpAssetsNonCurrent = compare
            ? compare.assets.rows.filter((r) => r.reportSection === 'ASSETS_NON_CURRENT')
            : undefined;
        const cmpAssetsCurrent = compare
            ? compare.assets.rows.filter((r) => r.reportSection === 'ASSETS_CURRENT')
            : undefined;
        const cmpLiabNonCurrent = compare
            ? compare.liabilities.rows.filter((r) => r.reportSection === 'LIABILITIES_NON_CURRENT')
            : undefined;
        const cmpLiabCurrent = compare
            ? compare.liabilities.rows.filter((r) => r.reportSection === 'LIABILITIES_CURRENT')
            : undefined;
        return {
            reportType: 'BS',
            title: 'Balance Sheet',
            ...
            compareOmittedReason,
            sections: [
                section('assets_non_current', 'Non-current assets', curAssetsNonCurrent, 'Total non-current assets', cmpAssetsNonCurrent, compare
                    ? (cmpAssetsNonCurrent?.reduce((s, r) => s + r.balance, 0) ?? 0)
                    : undefined),
                section('assets_current', 'Current assets', curAssetsCurrent, 'Total current assets', cmpAssetsCurrent, compare
                ...
                section('equity', 'Equity', current.equity.rows, 'Total equity', compare ? compare.equity.rows : undefined, compare ? compare.equity.total : undefined),
                section('liabilities_non_current', 'Non-current liabilities', curLiabNonCurrent, 'Total non-current liabilities', cmpLiabNonCurrent, compare
                    ? (cmpLiabNonCurrent?.reduce((s, r) => s + r.balance, 0) ?? 0)
                    : undefined),
                section('liabilities_current', 'Current liabilities', curLiabCurrent, 'Total current liabilities', cmpLiabCurrent, compare
    ...
    async presentCF(req, params) {
        ...
        const opRows = [
            ...
            ...current.operating.adjustments.map((a) => ({
                ...
                compareAmount: compare
                    ? this.present(compare.operating.adjustments.find((x) => x.label === a.label)
                        ?.amount ?? 0)
                    : undefined,
            })),
            ...current.operating.workingCapital.map((a) => ({
                ...
                amount: this.present(a.amount),
                compareAmount: compare
                    ? this.present(compare.operating.workingCapital.find((x) => x.label === a.label)
                        ?.amount ?? 0)
                    : undefined,
            })),
        ];
        const invRows = (current.investing.rows ?? []).map((r) => ({
            key: `inv:${r.label}`,
            label: r.label,
            amount: this.present(r.amount),
            compareAmount: compare
                ? this.present((compare.investing.rows ?? []).find((x) => x.label === r.label)
                    ?.amount ?? 0)
                : undefined,
        }));
        const finRows = (current.financing.rows ?? []).map((r) => ({
            key: `fin:${r.label}`,
            label: r.label,
            amount: this.present(r.amount),
            compareAmount: compare
                ? this.present((compare.financing.rows ?? []).find((x) => x.label === r.label)
                    ?.amount ?? 0)
                : undefined,
        }));
        const netChangeFromSections = current.operating.netCashFromOperating +
            ...
            sections: [
                {
                    ...
                    subtotal: {
                        key: 'net_cash_ops',
                        label: current.operating.netCashFromOperating >= 0
...
exports.ReportPresentationService = ReportPresentationService;
exports.ReportPresentationService = ReportPresentationService = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [financial_statements_service_1.FinancialStatementsService])
], ReportPresentationService);
//# sourceMappingURL=report-presentation.service.js.map