...
let FinanceArInvoicesService = class FinanceArInvoicesService {
    ...
    async getById(req, id) {
        ...
        return {
            ...
            lines: (inv.lines ?? []).map((l) => ({
                ...
                discountAmount: l.discountAmount === null || l.discountAmount === undefined
                    ? l.discountAmount
                    : Number(l.discountAmount),
                discountTotal: l.discountTotal === null || l.discountTotal === undefined
                    ? l.discountTotal
    ...
    async post(req, id, opts) {
        ...
        const revenueAccountIds = [...new Set(lines.map((l) => l.accountId).filter(Boolean))];
        ...
        const revenueAccountById = new Map(revenueAccounts.map((a) => [a.id, a]));
        const tenantRules = await this.prisma.tenant.findUnique({
            where: { id: tenant.id },
            select: {
        ...
        for (let i = 0; i < lines.length; i++) {
            const l = lines[i];
            const acct = revenueAccountById.get(l.accountId);
            const missing = [];
            ...
            if (needsDept && !l.departmentId)
                missing.push('department');
            if (needsProj && !l.projectId)
                missing.push('project');
            if (needsFund && !l.fundId)
                missing.push('fund');
            if (missing.length > 0) {
                issues.push({
                    lineNumber: i + 1,
        ...
        try {
                        ...
                        create: [
                            {
                                ...
                                credit: 0,
                                departmentId: lines[0]?.departmentId ?? null,
                                projectId: lines[0]?.projectId ?? null,
                                fundId: lines[0]?.fundId ?? null,
                            },
                            ...lines.map((l) => ({
                                accountId: l.accountId,
                                debit: 0,
            ...
            await this.prisma.auditEvent
                ...
                .catch(() => undefined);
            return {
                invoice: await this.getById(req, updated.id),
    ...
    async bulkPost(req, dto) {
        ...
        this.ensureUser(req);
        const ids = Array.from(new Set((dto.invoiceIds ?? [])
            .map((x) => String(x ?? '').trim())
            .filter(Boolean)));
        if (ids.length < 1)
    ...
    async previewImport(req, file, opts) {
        ...
        const parsedAll = rows.map((r) => {
            ...
            const revenueAccountCode = String(raw.revenueaccountcode ??
        ...
        for (const [invoiceRef, rows] of groups.entries()) {
            ...
            const mixedCurrency = rows.some((r) => String(r.currency ?? '')
                .trim()
    ...
    async import(req, file, importIdRaw) {
        ...
        const customerCodes = [
            ...new Set(validRows.map((r) => r.customerCode)),
        ];
        const accountCodes = [
            ...new Set(validRows.map((r) => r.revenueAccountCode)),
        ];
        ...
        for (const [, rows] of groups.entries()) {
            try {
                ...
                const tenantCurrency = String(tenant?.defaultCurrency ?? '').trim();
                const exchangeRate = tenantCurrency &&
  ...
  <body>
    ...
    ${invoiceNote ? `<div class="block"><div class="card"><div style="font-weight: 700;">Note</div><div style="margin-top: 6px; white-space: pre-wrap;">${this.escapeHtml(invoiceNote)}</div></div></div>` : ''}
        ...
        if (opts.format === 'pdf') {
            ...
            const billToEmail = inv.customerEmailSnapshot
                ? String(inv.customerEmailSnapshot).trim()
                : '';
