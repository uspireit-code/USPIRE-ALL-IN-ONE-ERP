...
let CoaService = class CoaService {
    ...
    async readXlsxRows(buf, sheetName) {
        const wb = new exceljs_1.default.Workbook();
        await wb.xlsx.load(buf);
        const ws = wb.worksheets.find((s) => s.name.trim().toLowerCase() === sheetName.trim().toLowerCase());
        if (!ws)
            return {
                headers: [],
                rows: [],
        ...
        headerRow.eachCell({ includeEmpty: true }, (cell, colNumber) => {
            const v = this.normalizeHeaderKey(cell.value?.text ?? cell.value);
        ...
        for (let r = 2; r <= ws.rowCount; r++) {
            ...
            if (hasAny)
                rows.push({ rowNumber: r, row: obj });
        }
        return { headers, rows };
    }
    mapCategoryStrict(raw) {
        const v = String(raw ?? '')
            .trim()
            .toUpperCase();
        if (v === 'ASSET')
    ...
    computeCanonicalHash(codes) {
        const sorted = [...codes]
            .map((c) => c.trim())
            .filter(Boolean)
            .sort();
        return (0, crypto_1.createHash)('sha256').update(sorted.join('\n')).digest('hex');
    }
    mapCategoryToAccountType(raw) {
        const v = String(raw ?? '')
            .trim()
            .toLowerCase();
        if (v === 'assets' || v === 'asset')
            return 'ASSET';
        if (v === 'liabilities' || v === 'liability')
            return 'LIABILITY';
        if (v === 'equity')
            return 'EQUITY';
    ...
    async getTenantCoaState(tenantId) {
    ...
    async importCanonical(req, file) {
        ...
        if (isCsv) {
            ...
            const found = new Set(parsedCsv.headers.filter(Boolean));
            ...
            const hasAllRequiredHeaders = (ws) => {
                const headerRow = ws.getRow(1);
                const found = new Set();
                headerRow.eachCell({ includeEmpty: true }, (cell) => {
                    const k = this.normalizeHeaderKey(cell.value?.text ?? cell.value);
                    if (k)
                        found.add(k);
                });
            ...
            if (!candidate) {
                errors.push({
                    row: 0,
                    column: '',
                    message: 'No worksheet contains the required COA columns. Required columns: accountCode, accountName, category, subCategory, normalBalance, fsMappingLevel1, fsMappingLevel2',
                });
            }
            else {
                ...
                const headers = [];
                headerRow.eachCell({ includeEmpty: true }, (cell, colNumber) => {
                    headers[colNumber - 1] = this.normalizeHeaderKey(cell.value?.text ?? cell.value);
                });
                for (let r = 2; r <= candidate.rowCount; r++) {
                    ...
                    const hasAny = Object.values(obj).some((v) => String(v ?? '').trim() !== '');
                    if (hasAny)
                        rawRows.push({ rowNumber: r, row: obj });
                }
            }
        }
        ...
        const parentCodesRequested = new Set();
        for (const r of rawRows) {
            ...
            const normalBalance = this.mapNormalBalanceStrict(String(balanceRaw ?? '').trim());
            ...
            if (parentAccountCode)
                parentCodesRequested.add(parentAccountCode);
            if (!accountCode)
                errors.push({
                    row: rowNumber,
                    column: 'accountCode',
            ...
            if (accountCode) {
                ...
                if (seenCodes.has(key))
                    duplicateCodes.add(accountCode);
                seenCodes.add(key);
            }
            if (accountCode &&
                accountName &&
        ...
        await this.prisma.$transaction(async (tx) => {
            ...
            await tx.auditEvent
                ...
                data: {
                    tenantId: tenant.id,
                    eventType: client_1.AuditEventType.COA_IMPORTED,
                    entityType: client_1.AuditEntityType.CHART_OF_ACCOUNTS,
                    entityId: tenant.id,
                    action: 'COA_IMPORTED',
                    outcome: 'SUCCESS',
        ...
        return {
            ...
            created: created.length,
            updated: updated.length,
            warnings: updated.length > 0
                ? [`${updated.length} accounts already existed and were updated`]
                : [],
        };
    }
    async getImportTemplate(req, params) {
        ...
        throw new common_1.BadRequestException('Unsupported format. Use csv or xlsx');
    }
    async cleanupNonCanonical(req, dto) {
        ...
        const canonicalCodes = Array.isArray(snapshot.data?.accountCodes)
            ? snapshot.data.accountCodes
            : [];
        const canonicalSet = new Set(canonicalCodes.map((c) => String(c ?? '').trim()));
        ...
        const addBlocked = (accountId, reason) => {
            const set = blockedById.get(accountId) ?? new Set();
            set.add(reason);
            blockedById.set(accountId, set);
        };
        for (const c of candidates) {
            if (c.isControlAccount)
                addBlocked(c.id, 'CONTROL_ACCOUNT');
        }
        ...
        const [journalRefs, budgetRefs, forecastRefs, supplierRefs, customerRefs, recurringRefs, bankRefs, taxRefs, faCatRefs, faRefs,] = await this.prisma.$transaction([
        ...
        for (const r of journalRefs)
            addBlocked(r.accountId, 'JournalLine');
        for (const r of budgetRefs)
            addBlocked(r.accountId, 'BudgetLine');
        for (const r of forecastRefs)
            addBlocked(r.accountId, 'ForecastLine');
        ...
        for (const r of bankRefs)
            addBlocked(r.glAccountId, 'BankAccount');
        for (const r of taxRefs)
            addBlocked(r.glAccountId, 'TaxRate');
        ...
        for (const r of faRefs) {
            if (r.assetAccountId)
                addBlocked(r.assetAccountId, 'FixedAsset');
            if (r.accumDepAccountId)
                addBlocked(r.accumDepAccountId, 'FixedAsset');
        ...
        await this.prisma.auditEvent
            ...
            data: {
                tenantId: tenant.id,
                eventType: client_1.AuditEventType.COA_CLEANUP_EXECUTED,
                entityType: client_1.AuditEntityType.CHART_OF_ACCOUNTS,
                entityId: tenant.id,
    ...
    async assertCoaNotFrozen(tenantId) {
        const { coaFrozen } = await this.getTenantCoaState(tenantId);
        if (coaFrozen)
            throw new common_1.ForbiddenException('Chart of Accounts is frozen');
    }
    async assertCoaNotLocked(params) {
        const { coaLockedAt } = await this.getTenantCoaState(params.tenantId);
        if (!coaLockedAt)
            return;
        if (params.operation === 'create') {
            throw new common_1.ForbiddenException('Chart of Accounts is locked: cannot create accounts');
        }
        if (params.operation === 'update_code') {
            throw new common_1.ForbiddenException('Chart of Accounts is locked: cannot edit account codes');
        }
        if (params.operation === 'update_hierarchy') {
            throw new common_1.ForbiddenException('Chart of Accounts is locked: cannot edit account hierarchy');
        }
    }
    async assertParentValid(params) {
        ...
        if (parent.isPosting) {
            throw new common_1.BadRequestException('Parent account cannot be a posting account');
        }
    }
    async assertCannotDeactivateUsedInPostedJournals(params) {
        ...
        if (used) {
            throw new common_1.BadRequestException('Cannot deactivate an account used in posted journals');
        }
    }
    async assertControlAccountMayDeactivate(params) {
        const [supplierLine, customerLine, bankAccount] = await this.prisma.$transaction([
        ...
        if (supplierLine || customerLine || bankAccount) {
            throw new common_1.BadRequestException('Control accounts linked to subledgers cannot be disabled');
        }
    }
    async assertNoChildren(params) {
        const child = await this.prisma.account.findFirst({
            where: { tenantId: params.tenantId, parentAccountId: params.accountId },
            select: { id: true },
        });
        if (child)
            throw new common_1.BadRequestException('Posting accounts cannot have child accounts');
    }
    async computeHierarchyPath(params) {
    ...
    async rebuildHierarchyPaths(params) {
        const rows = await this.prisma.account.findMany({
        ...
        await this.prisma.$transaction(updates.map((u) => this.prisma.account.updateMany({
            where: { tenantId: params.tenantId, id: u.id },
            data: { hierarchyPath: u.hierarchyPath },
        })));
    }
    async assertNoCircularReference(params) {
        if (params.parentAccountId === params.accountId) {
            throw new common_1.BadRequestException('Account cannot be its own parent');
        }
        let cursor = params.parentAccountId;
        const seen = new Set();
        while (cursor) {
            if (seen.has(cursor)) {
                throw new common_1.BadRequestException('Circular account hierarchy detected');
            }
            seen.add(cursor);
    ...
    async create(req, dto) {
        ...
        const created = await this.prisma.account.create({
            data: {
                ...
                normalBalance: dto.normalBalance ?? 'DEBIT',
                isActive: dto.isActive ?? true,
                isBudgetRelevant: dto.isBudgetRelevant,
                budgetControlMode: dto.budgetControlMode ?? undefined,
                createdById: user.id,
            },
            select: { id: true },
        });
    ...
    async setupTaxControlAccounts(req) {
        ...
        await this.prisma.$transaction(async (tx) => {
            for (const spec of specs) {
                ...
                if (existingByCode) {
                    throw new common_1.BadRequestException(`Cannot create tax control account ${spec.ifrsMappingCode}: account code '${spec.code}' already exists with ifrsMappingCode '${existingByCode.ifrsMappingCode ?? ''}'`);
                }
    ...
    async update(req, id, dto) {
        ...
        if (dto.ifrsMappingCode !== undefined &&
        ...
        const newParent = parentAccountId === undefined ? oldParent : parentAccountId;
        const hierarchyChanged = newParent !== oldParent;
        const postingPatch = requestedPosting === undefined
            ? {}
            : {
        ...
        const res = await this.prisma.account.updateMany({
            ...
            data: {
                ...
                normalBalance: dto.normalBalance ?? undefined,
        ...
        const isDeactivate = dto.isActive === false &&
        ...
        const mappingChanged = dto.ifrsMappingCode !== undefined &&
            String(dto.ifrsMappingCode ?? '') !==
                String(existing.ifrsMappingCode ?? '');
        if (mappingChanged) {
            await this.prisma.auditEvent
