  ...
  <body>
    ...
    <div class="block">
      <div class="card">
        ...
        if (customerEmail)
            doc.font('Helvetica').fontSize(9).fillColor(colors.muted).text(customerEmail);
        if (customerPhone)
            doc.font('Helvetica').fontSize(9).fillColor(colors.muted).text(customerPhone);
        if (customerAddress) {
        ...
        const tableWidth = cols.invoice + cols.invDate + cols.dueDate + cols.invTotal + cols.applied + cols.balance;
        ...
        const renderRow = (row) => {
            ...
            const y = doc.y;
            const cellPadY = 6;
            const maxH = Math.max(doc.heightOfString(row.invoiceNumber || '', { width: cols.invoice - 8 }), doc.heightOfString(String(row.invoiceDate ?? ''), { width: cols.invDate - 8 }), doc.heightOfString(String(row.dueDate ?? ''), { width: cols.dueDate - 8 }));
    ...
    async updateReceipt(req, id, dto) {
        ...
        const nextExchangeRate = dto.exchangeRate === undefined ? Number(current.exchangeRate ?? 1) : dto.exchangeRate;
        if (dto.customerId) {
            const customer = await this.prisma.customer.findFirst({
                where: { id: dto.customerId, tenantId: tenant.id },
                select: { id: true, status: true },
    ...
    async postReceipt(req, id) {
        ...
        const unappliedReceiptsAccountId = (tenantConfig?.unappliedReceiptsAccountId ?? null);
