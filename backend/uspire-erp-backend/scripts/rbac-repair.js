...
async function main() {
    ...
    console.log(`[rbac-repair] Ensured FINANCE_CONTROLLER has ${glFinalPostCode}. Tenants=${tenants.length}, rows inserted=${controllerInsert.count}.`);
    for (const row of controllerRoleByTenant) {
        console.log(`[rbac-repair] tenantId=${row.tenantId} financeControllerRoleId=${row.roleId} hasFinalPost=${mappedRoleIds.has(row.roleId)}`);
    }
}
main()
    .catch((e) => {
...
//# sourceMappingURL=rbac-repair.js.map