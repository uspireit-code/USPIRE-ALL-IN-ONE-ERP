...
## RBAC runtime repair (Finance Manager GL Review Queue)

If an existing database is missing the `FINANCE_GL_APPROVE` mapping for the `FINANCE_MANAGER` role, you can apply an additive, idempotent repair:
...
3. Logout/login as Finance Manager (JWT refresh)
4. Confirm `/auth/me` includes `FINANCE_GL_APPROVE`
5. Confirm `/finance/gl/review` loads successfully
